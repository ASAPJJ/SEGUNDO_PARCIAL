...
app.get('/check-mysql-connection', async (req, res) => {
  try {
    ...
    res.json({ status: 'success', message: 'La conexión a MySQL se ha establecido correctamente.' });
  } catch (error) {
...
app.get('/check-mongodb-connection', (req, res) => {
  ...
  if (db.readyState === 1) {
    res.json({ status: 'success', message: 'La conexión a MongoDB se ha establecido correctamente.' });
  } else {
